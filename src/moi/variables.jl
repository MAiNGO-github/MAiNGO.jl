...
MOI.supports_add_constrained_variable(model::Optimizer, set::MOI.Reals) = true
...
MOI.supports_add_constrained_variable(model::Optimizer, set::MOI.Integer) = true

MOI.supports_add_constrained_variable(model::Optimizer, set::MOI.ZeroOne) = true
...
function MOI.add_constrained_variable(model::Optimizer, set::MOI.Interval)
    ...
    MOI.ConstraintIndex{MOI.VariableIndex,MOI.Interval{Float64}}(
        length(model.inner.variable_info),
    )
end
...
function MOI.add_constraint(
    ...
    model.inner.variable_info[vi.value].upper_bound =
        min(model.inner.variable_info[vi.value].upper_bound, lt.upper)
...
function MOI.add_constraint(
    ...
    model.inner.variable_info[vi.value].lower_bound =
        max(model.inner.variable_info[vi.value].lower_bound, gt.lower)
    if (model.inner.variable_info[vi.value].category == :Bin)
        #Bin can not have bounds
        model.inner.variable_info[vi.value].category = :Int
    end
...
function MOI.add_constraint(
    ...
    model.inner.variable_info[vi.value].lower_bound =
        max(model.inner.variable_info[vi.value].lower_bound, eq.value)
    model.inner.variable_info[vi.value].upper_bound =
        min(model.inner.variable_info[vi.value].upper_bound, eq.value)
    if (model.inner.variable_info[vi.value].category == :Bin)
        #Bin can not have bounds
        model.inner.variable_info[vi.value].category = :Int
    end
